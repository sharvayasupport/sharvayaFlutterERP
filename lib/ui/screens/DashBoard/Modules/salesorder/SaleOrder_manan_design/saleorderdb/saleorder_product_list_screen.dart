...
class AddSalesOrderProductListArgument {
  String quotation_No;
  String stateCode;
  String HeaderDiscAmnt;

  AddSalesOrderProductListArgument(
      this.quotation_No, this.stateCode, this.HeaderDiscAmnt);
}

class SalesOrderProductListScreen extends BaseStatefulWidget {
  static const routeName = '/SalesOrderProductListScreen';
  final AddSalesOrderProductListArgument arguments;
  SalesOrderProductListScreen(this.arguments);
  @override
  _SalesOrderProductListScreenState createState() =>
      _SalesOrderProductListScreenState();
}

class _SalesOrderProductListScreenState
    extends BaseState<SalesOrderProductListScreen>
        ...
        children: [
          ...
          FloatingActionButton(
            ...
            onPressed: () async {
              ...
              await navigateTo(context, AddSalesOrderProductScreen.routeName,
                  arguments: AddSalesOrderProductScreenArguments(
  ...
  Future<void> UpdateAfterHeaderDiscountToDB() async {
    ...
    print(
        "Tot_NetAmtTot_NetAmt" + " Tot_NetAmt : " + _HeaderDiscAmnt.toString());

    if (_HeaderDiscAmnt == "") {
      _HeaderDiscAmnt = "0.00";
    }
    double HeaderDisAmnt = double.parse(
        _HeaderDiscAmnt == null || _HeaderDiscAmnt == ""
            ? 0.00
            : _HeaderDiscAmnt);
  ...
  Future<void> _onTapOfDeleteContact(int index) async {
    await OfflineDbHelper.getInstance()
        .deleteSalesOrderProduct(_productList[index].id);
    setState(() {
