...
class SearchSourceScreen extends BaseStatefulWidget {
  static const routeName = '/SearchSourceScreen';

  @override
  _SearchSourceScreenState createState() => _SearchSourceScreenState();
}

class _SearchSourceScreenState extends BaseState<SearchSourceScreen>
    ...
    return BlocProvider(
      ...
      child: BlocConsumer<GoogleMapScreenBloc, GoogleMapScreenStates>(
        builder: (BuildContext context, GoogleMapScreenStates state) {
          if (state is PlaceSearchResponseState) {
            _onSearchInquiryListCallSuccess(state);
          }
          return super.build(context);
        },
        buildWhen: (oldState, currentState) {
          if (currentState is PlaceSearchResponseState) {
            return true;
          }
          return false;
        },
  ...
  Widget buildBody(BuildContext context) {
    return Column(
      children: [
        NewGradientAppBar(
          title: Text('Search Source Details'),
    ...
    return Column(
      ...
      children: [
        Container(
          ...
          child: Text("Min. 3 chars to search Area",
              style: TextStyle(
                  fontSize: 12,
                  color: colorPrimary,
  ...
  Widget _buildInquiryList() {
    ...
    return ListView.builder(
      ...
      shrinkWrap: true,
      itemCount: placesSearchResponse.results.length,
    );
  }

  ///builds row item view of inquiry list
  Widget _buildSearchInquiryListItem(int index) {
    SearchGoogleResultResults model = placesSearchResponse.results[index];
  ...
  void _onSearchChanged(String value) {
    if (value.trim().length > 2) {
      _googleMapScreenBloc.add(PlaceSearchRequestCallEvent(
          MapAPIKey, PlaceSearchRequest(query: value)));
    }
  }

  void _onSearchInquiryListCallSuccess(PlaceSearchResponseState state) {
    placesSearchResponse = state.placesSearchResponse;
  }
}
